...
import { ensureServerSecretsLoaded, getServerSecretValue } from "@/lib/server/secret-manager";
...
async function getProviderSelection(options: EmailProviderOptions = {}): Promise<EmailProviderSelection> {
  ...
  const emailUser = await getServerSecretValue("EMAIL_USER");
  const emailPass = await getServerSecretValue("EMAIL_PASS");
